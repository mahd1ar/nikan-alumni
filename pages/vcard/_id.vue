...
export default Vue.extend({
  ...
  asyncData(args) {
    ...
    args.redirect('https://nikan.nikpeyvand.com' + path)
  },
  data() {
    return {
      loaded: false,
  ...
  async fetch() {
    ...
    const { data } = await this.$axios.get<WPapi.pulicUser.RootObject | false>(
      'https://cors.nikan-alumni.com/https://nikan-alumni.org/wp-json/myplugin/v1/user/' +
        String(+id) +
        '/'
    )
    ...
    const { instagram, linkedin, twitter, city, providence } =
