<template>
  <div dir="rtl" class="relative flex min-h-screen flex-col justify-center overflow-hidden bg-gray-50 py-6 sm:py-40">
    <img class="absolute top-0 left-0" style="transform: scale(-1, 1)"
      src="https://netita.ir/html/vcard/assets/images/triangles-top.png" alt="" />
    <img class="absolute bottom-0 right-0" style="transform: scale(-1, 1)"
      src="https://netita.ir/html/vcard/assets/images/triangles-bottom.png" alt="" />

    <div class="container relative top-20 mx-auto flex flex-col-reverse items-start gap-7 sm:top-0 md:flex-row">
      <div class="flex w-full flex-col overflow-hidden rounded-xl bg-white md:w-7/12 lg:w-9/12">
        ...
        <div class="h-full bg-slate-100 px-7 pb-7">
          ...
          <div class="my-3 grid grid-cols-1 gap-2 lg:grid-cols-2">
            <div class="flex cursor-pointer overflow-hidden rounded bg-white p-3 shadow-sm hover:bg-gray-50">
              ...
              <a :href="'tel:' + user.mobile" target="_system" class="relative px-2 text-slate-700">
                <span class="relative flex items-center justify-start gap-2 text-lg font-bold">
                  <span> تماس با تلفن همراه </span>
                  <svg class="relative top-0.5" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img"
            ...
            <!-- // phone -->
            <div v-if="user.phone" :title="user.phone"
              ...
              <a :href="'tel:' + user.phone" class="relative px-2 text-slate-700">
                <span class="relative flex items-center justify-start gap-2 text-lg font-bold">
            ...
            <div :title="user.email"
              ...
              <a :href="`mailto:${user.email}`" class="relative px-2 text-slate-700">
                <span class="relative flex items-center justify-start gap-2 text-lg font-bold">
                ...
                <span class="inline-block w-full overflow-hidden text-ellipsis whitespace-nowrap">
                  {{ user.email }}
                </span>
              </a>
            </div>

            <a v-if="user.website" :href="user.website" :title="user.website" target="_blank"
              class="flex cursor-pointer overflow-hidden rounded bg-white p-3 shadow-sm hover:bg-gray-50">
              <div class="rounded-lg bg-cyan-50 p-3 text-cyan-400">
                <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" class="h-8 w-8"
                  preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
          ...
          "
            ...
            <div class="flex items-start">
              <div class="rounded-lg bg-cyan-50 p-3 text-cyan-400">
                <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" class="h-8 w-8 shrink-0"
            ...
            <div v-if="+user.jobLocation.lat !== 0" class="h-72 py-5 px-3">
              <map-picker :lat="user.jobLocation.lat" :lng="user.jobLocation.lng" :zoom="13" :disabled="true">
              </map-picker>
            </div>
          </div>
        </div>
      ...
      <div class="flex w-full flex-col rounded-xl bg-white md:w-5/12 lg:w-3/12">
        <div class="px-7">
          <div class="relative mx-auto flex h-20 max-w-xs items-center justify-center">
            <!-- src="https://avatars.githubusercontent.com/u/32998122?v=4" -->
            <img v-if="loaded && !$fetchState.pending" loading="lazy" :src="user.avatar"
              class="absolute bottom-0 h-40 w-40 rounded-3xl" alt="" />
          </div>
          ...
          <div class="mb-3 flex items-center justify-center gap-3 py-3 text-gray-700">
            <a v-if="user.socialMedias && user.socialMedias.twitter" :href="user.socialMedias.twitter" target="_blank">
            ...
            <a v-if="user.socialMedias && user.socialMedias.linkedin" :href="user.socialMedias.linkedin"
              target="_blank">
              <svg aria-hidden="true" role="img" class="w-5 h-5" preserveAspectRatio="xMidYMid meet"
            ...
            <!-- instagram -->
            <a v-if="user.socialMedias && user.socialMedias.instagram" :href="user.socialMedias.instagram"
              target="_blank">
              <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" class="h-5 w-5"
                preserveAspectRatio="xMidYMid meet" viewBox="0 0 1024 1024">
                <path fill="currentColor"
        ...
        <div class="mt-auto flex shrink-0 flex-col gap-3 rounded-b-xl bg-slate-100 px-7 py-8 text-[#44566c]">
          <div v-if="user.location" class="flex items-center gap-1">
            <svg v-if="user.location.city || user.location.providence" xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true" role="img" class="h-5 w-5 shrink-0" preserveAspectRatio="xMidYMid meet"
              viewBox="0 0 1024 1024">
              <path fill="currentColor"
                d="M800 416a288 288 0 1 0-576 0c0 118.144 94.528 272.128 288 456.576C705.472 688.128 800 534.144 800 416zM512 960C277.312 746.688 160 565.312 160 416a352 352 0 0 1 704 0c0 149.312-117.312 330.688-352 544z" />
              <path fill="currentColor"
          ...
          <div class="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" class="h-5 w-5 shrink-0"
              preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
          ...
          <a v-if="user.mobile" download target="_blank" :href="contact"
...
export default Vue.extend({
  ...
  async fetch() {
    ...
    const { data } = await this.$axios.get<WPapi.pulicUser.RootObject | false>(
      'https://cors.nikan-alumni.com/https://nikan-alumni.org/wp-json/myplugin/v1/user/' + String( +id) + "/"
    )
